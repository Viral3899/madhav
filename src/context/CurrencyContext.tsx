...
export function CurrencyProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('madhav_currency') as CurrencyCode | null;
    const savedCountry = localStorage.getItem('madhav_country') as Country | null;
    if (saved && currencies[saved]) setCurrency(saved);
    if (savedCountry && countries[savedCountry]) setCountryState(savedCountry);
  }, []);
  function changeCurrency(code: CurrencyCode) {
    setCurrency(code);
    localStorage.setItem('madhav_currency', code);
  }
  function changeCountry(value: Country) {
    setCountryState(value);
    changeCurrency(countries[value]);
    localStorage.setItem('madhav_country', value);
  }
...
export const currencyOptions = Object.entries(currencies).map(([code, value]) => ({
