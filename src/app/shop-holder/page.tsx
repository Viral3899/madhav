...
export default function ShopHolderPage() {
  ...
  useEffect(() => {
    if (user?.role === 'seller') window.location.href = '/seller';
  }, [user]);
    ...
    return (
      <main className="min-h-screen grid place-items-center p-6">
        <div className="text-center">
          ...
          <h1 className="font-display text-4xl italic mb-4">Shop holder signed in</h1>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link className="btn-primary rounded-sm" href="/admin">
    ...
    <main className="min-h-screen bg-secondary/40 grid place-items-center p-6">
      <div className="w-full max-w-md bg-background border border-border rounded-2xl p-7 shadow-xl">
        ...
        <h1 className="font-display text-4xl italic mt-8 mb-2">
          Shop holder {mode === 'login' ? 'sign in' : 'registration'}
        </h1>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          {mode === 'register' && (
            <input
              ...
              placeholder="Shop-holder setup key"
              className="w-full checkout-input"
            />
          )}
          ...
          <button disabled={busy} className="btn-primary w-full justify-center rounded-sm">
            {busy
              ? 'Please wait...'
              : mode === 'login'
                ? 'Sign in as shop holder'
                : 'Create shop-holder account'}
          </button>
        </form>
        <p className="text-xs text-muted-foreground mt-6">
          Customers should use the normal sign-in/register controls on the storefront.
        </p>
